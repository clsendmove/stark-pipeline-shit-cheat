...
namespace character {
    template <typename type> constexpr bool isUpper(const type character) {
        return (character >= static_cast<const type>(65) && character <= static_cast<const type>(90));
    }

    template <typename type> constexpr type toLower(const type character) {
    ...
    template <typename type> constexpr bool isTerminator(const type character) {
        return (character == static_cast<const type>(0));
    }

    template <typename type> constexpr bool isQuestion(const type character) {
        return (character == static_cast<const type>(63));
    }

    template <typename type> constexpr size_t getLength(const type* const data) {
        size_t length{ 0 };
